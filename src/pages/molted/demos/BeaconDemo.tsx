...
import { Send, RefreshCw, Copy, Check, AlertTriangle, Sparkles, User, MessageSquare } from 'lucide-react';

const BLUE = '#1E3A8A';
const BLUE_DIM = 'rgba(30,58,138,0.12)';
const BLUE_BORDER = 'rgba(30,58,138,0.30)';
const BLUE_GLOW = '0 0 40px rgba(30,58,138,0.20)';
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      style={{ background: BLUE_DIM, color: copied ? '#22c55e' : '#94a3b8', border: `1px solid ${BLUE_BORDER}` }}
    >
...
function ToneBadge({ tone }: { tone: string }) {
  const colors: Record<string, string> = {
    encouraging: '#22c55e', empathetic: '#a78bfa', informational: '#38bdf8',
    celebratory: '#f59e0b', supportive: '#a78bfa', reassuring: '#22c55e',
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: '#0a0a0f', color: '#f1f5f9' }}>
      ...
      <div className="border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/beacon" className="text-sm text-slate-500 hover:text-slate-300 transition-colors">Beacon</Link>
            <span className="text-slate-700">/</span>
            <span className="text-sm font-semibold" style={{ color: `rgb(${BLUE.slice(1).match(/../g)?.map(h => parseInt(h,16)).join(',')})` }}>Interactive Demo</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold"
            style={{ background: BLUE_DIM, border: `1px solid ${BLUE_BORDER}`, color: '#60a5fa' }}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        {/* Title */}
        <div className="mb-10">
          <h1 className="text-3xl font-black tracking-tight mb-2">Beacon — Institutional Voice AI</h1>
          <p className="text-slate-400">See how your institution's AI persona responds to real student messages — in your voice, with your values.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {PERSONAS.map(p => (
                  <button key={p.id} onClick={() => setSelectedPersona(p)}
                    ...
                    style={{
                      background: selectedPersona.id === p.id ? BLUE_DIM : 'rgba(255,255,255,0.03)',
                      border: `1px solid ${selectedPersona.id === p.id ? BLUE_BORDER : 'rgba(255,255,255,0.07)'}`,
                      boxShadow: selectedPersona.id === p.id ? BLUE_GLOW : 'none',
                    }}>
                    <div className="text-sm font-bold text-slate-200 mb-1">{p.id.charAt(0).toUpperCase() + p.id.slice(1)}</div>
                ...
                {TOUCHPOINTS.map(t => (
                  <button key={t} onClick={() => setSelectedTouchpoint(t)}
                    ...
                    style={{
                      ...
                      border: `1px solid ${selectedTouchpoint === t ? BLUE_BORDER : 'rgba(255,255,255,0.1)'}`,
                      color: selectedTouchpoint === t ? '#93c5fd' : '#64748b',
                    }}>
                    {t}
                  </button>
                ))}
            ...
            <button
              ...
              style={{
                background: loading || !message.trim() ? 'rgba(255,255,255,0.05)' : `linear-gradient(135deg, #1e3a8a, #2563eb)`,
                color: loading || !message.trim() ? '#475569' : '#fff',
                boxShadow: loading || !message.trim() ? 'none' : BLUE_GLOW,
              }}>
          ...
          <div>
            {!result && !loading && (
              <div className="h-full flex flex-col items-center justify-center text-center py-20"
                ...
                <p className="text-slate-600 text-sm mt-1">Select a persona and send a student message</p>
              </div>
            )}

            {loading && (
              <div className="h-full flex flex-col items-center justify-center text-center py-20"
                style={{ border: '1px solid rgba(30,58,138,0.2)', borderRadius: '1.25rem', background: BLUE_DIM }}>
                <RefreshCw size={32} className="animate-spin mb-4" style={{ color: '#60a5fa' }} />
                <p className="font-medium text-slate-300">{selectedPersona.name.split('—')[0].trim()} is composing a response...</p>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                {result.escalate && (
                  <div className="p-4 rounded-xl flex items-start gap-3"
                    ...
                    <div>
                      <p className="text-sm font-bold text-red-300 mb-1">Escalation Flagged</p>
                      <p className="text-xs text-red-400">{result.escalateReason}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between px-4 py-3 rounded-xl"
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
                      style={{ background: BLUE_DIM, border: `1px solid ${BLUE_BORDER}`, color: '#93c5fd' }}>
                      {selectedPersona.id[0].toUpperCase()}
                    </div>
                    <span className="text-sm font-semibold text-slate-300">{selectedPersona.name}</span>
                  </div>
                ...
                <div className="p-5 rounded-xl" style={{ background: BLUE_DIM, border: `1px solid ${BLUE_BORDER}` }}>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-semibold uppercase tracking-widest text-blue-400">Beacon Response</span>
                    <CopyButton text={result.reply} />
                  </div>
                  <p className="text-slate-200 leading-relaxed text-sm">{result.reply}</p>
                </div>
                ...
                <div className="px-4 py-3 rounded-xl"
                  ...
                  <p className="text-sm text-slate-300">{result.intent}</p>
                </div>
                ...
                {result.faithNote && (
                  <div className="px-4 py-3 rounded-xl"
                    ...
                    <span className="text-xs font-semibold uppercase tracking-widest text-yellow-600 block mb-1 flex items-center gap-1">
                      <Sparkles size={11} /> Faith Integration
                    </span>
                    <p className="text-sm text-yellow-200/70">{result.faithNote}</p>
                  </div>
                )}
        ...
        <div className="mt-16 pt-8 border-t border-white/5 flex justify-between items-center">
          <Link to="/beacon" className="text-sm text-slate-500 hover:text-slate-300 transition-colors flex items-center gap-1">← Back to Beacon</Link>
          <Link to="/retain-ai/demo" className="text-sm font-semibold flex items-center gap-1" style={{ color: '#f43f5e' }}>
            Try RetainAI Demo →
          </Link>
        </div>
      </div>
