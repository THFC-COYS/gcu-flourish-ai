...
const ROSE = '#E11D48';
const ROSE_DIM = 'rgba(225,29,72,0.08)';
const ROSE_BORDER = 'rgba(225,29,72,0.22)';
const GREEN = '#16A34A';
const GREEN_DIM = 'rgba(22,163,74,0.08)';
const GREEN_BORDER = 'rgba(22,163,74,0.22)';
const AMBER = '#D97706';
const AMBER_DIM = 'rgba(217,119,6,0.08)';
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      style={{ background: CARD_INNER, color: copied ? GREEN : '#64748b', border: `1px solid ${BORDER}` }}>
...
function RiskMeter({ score }: { score: number }) {
  ...
  return (
    <div>
      ...
      <div className="h-3 rounded-full overflow-hidden" style={{ background: CARD_INNER }}>
...
function WeightBadge({ weight }: { weight: string }) {
  const color = weight === 'high' ? '#dc2626' : weight === 'medium' ? '#d97706' : '#64748b';
  return (
    <span className="px-2 py-0.5 rounded-full text-xs font-semibold capitalize"
      style={{ background: `${color}14`, color, border: `1px solid ${color}30` }}>{weight}</span>
  );
}

function StudentCard({ student, selected, onClick }: { student: typeof SAMPLE_STUDENTS[0]; selected: boolean; onClick: () => void }) {
  const riskColor = student.missedAssignments >= 5 ? '#dc2626' : student.missedAssignments >= 3 ? '#d97706' : '#16a34a';
  return (
    <button onClick={onClick}
      className="w-full p-4 rounded-xl text-left transition-all"
      style={{
        background: selected ? ROSE_DIM : CARD,
        border: `1px solid ${selected ? ROSE_BORDER : BORDER}`,
        boxShadow: selected ? '0 0 0 3px rgba(225,29,72,0.08)' : 'none',
      }}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold"
            style={{ background: `${riskColor}14`, color: riskColor }}>
            {student.name[0]}
          </div>
          <div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="px-2 py-1.5 rounded-lg" style={{ background: CARD_INNER }}>
          <p className="text-xs font-bold" style={{ color: student.gpa < 2 ? '#dc2626' : '#64748b' }}>{student.gpa}</p>
          <p className="text-xs text-slate-400">GPA</p>
        </div>
        <div className="px-2 py-1.5 rounded-lg" style={{ background: CARD_INNER }}>
          <p className="text-xs font-bold" style={{ color: student.missedAssignments > 4 ? '#dc2626' : '#d97706' }}>{student.missedAssignments}</p>
          <p className="text-xs text-slate-400">Missed</p>
        </div>
        <div className="px-2 py-1.5 rounded-lg" style={{ background: CARD_INNER }}>
          <p className="text-xs font-bold" style={{ color: student.loginDaysLast14 < 4 ? '#dc2626' : '#64748b' }}>{student.loginDaysLast14}</p>
          <p className="text-xs text-slate-400">Logins/14d</p>
        </div>
      </div>
    </button>
  );
...
function ModeToggle({ mode, onChange }: { mode: 'advisor' | 'auto'; onChange: (m: 'advisor' | 'auto') => void }) {
  return (
    <div
      ...
      style={{ background: CARD_INNER, border: `1px solid ${BORDER}` }}
    >
      <button
        ...
        style={{
          ...
          color: mode === 'advisor' ? AMBER : '#94a3b8',
...
function AutoInterventionMode() {
  ...
  const highRiskStudent = SAMPLE_STUDENTS[0];
  ...
  const urgencyColor: Record<string, string> = { today: '#dc2626', 'this week': '#d97706', 'this month': '#64748b' };
  const ownerColor: Record<string, string> = { advisor: '#7c3aed', system: '#0284c7', counselor: '#d97706' };
      ...
      <div className="p-5 rounded-2xl" style={{ background: ROSE_DIM, border: `1px solid ${ROSE_BORDER}` }}>
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          {[
            ...
            <div key={sub} className="text-center p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.7)' }}>
      ...
      {!started ? (
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <div ref={tracesRef} className="p-4 space-y-1.5 max-h-64 overflow-y-auto font-mono" style={{ background: CARD_INNER }}>
              {traces.map((t, i) => (
                <div key={i} className="flex items-center gap-2.5 text-xs">
                  ...
                  />
                  <span style={{ color: TRACE_COLORS[t.type] ?? '#475569' }}>{t.step}</span>
                </div>
              ))}
              {loading && !sent && (
          ...
          {sent && result && (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-slate-800">Outreach sent automatically</p>
          ...
          {result && (
            <div className="space-y-4">
              ...
              <div className="p-5 rounded-xl" style={{ background: ROSE_DIM, border: `1px solid ${ROSE_BORDER}` }}>
                ...
                <p className="text-xs text-slate-600 mt-3 leading-relaxed">{result.predictedOutcome}</p>
              </div>

              {/* Retention probability */}
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.15)' }}>
                  <p className="text-2xl font-black text-red-600">{result.retentionProbability.withoutIntervention}%</p>
                  <p className="text-xs text-slate-500 mt-1">Without intervention</p>
                </div>
                <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(22,163,74,0.06)', border: '1px solid rgba(22,163,74,0.15)' }}>
                  <p className="text-2xl font-black text-green-600">{result.retentionProbability.withIntervention}%</p>
                  <p className="text-xs text-slate-500 mt-1">With intervention</p>
                </div>
              </div>

              {/* Auto-dispatched email */}
              <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${GREEN_BORDER}` }}>
                <div className="flex items-center justify-between p-4" style={{ background: GREEN_DIM }}>
                  <div className="flex items-center gap-2">
                    <Mail size={14} style={{ color: GREEN }} />
                    <span className="text-sm font-semibold text-slate-700">Auto-Dispatched Outreach Email</span>
                ...
                <div className="px-4 pb-4 pt-3" style={{ background: '#f0fdf4' }}>
                  <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{result.emailDraft}</p>
                </div>
              </div>
    ...
    <div className="grid lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ...
              <div key={String(label)} className="flex justify-between items-center">
                ...
                <span className="font-semibold" style={{ color: alert ? '#dc2626' : '#1e293b' }}>{value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <button onClick={handleAnalyze} disabled={loading}
          ...
          style={{
            background: loading ? CARD_INNER : `linear-gradient(135deg, #be123c, #e11d48)`,
            color: loading ? '#94a3b8' : '#fff',
            boxShadow: loading ? 'none' : '0 4px 14px rgba(225,29,72,0.25)',
          }}>
      ...
      <div>
        {!result && !loading && (
          <div className="h-full flex flex-col items-center justify-center text-center py-24"
            ...
            <TrendingDown size={40} className="mb-4 text-slate-300" />
            <p className="text-slate-500 font-medium">Intervention plan will appear here</p>
            <p className="text-slate-400 text-sm mt-1">Select a student and click Analyze</p>
          </div>
        )}

        {loading && (
          <div className="h-full flex flex-col items-center justify-center text-center py-24"
            style={{ border: `1px solid ${ROSE_BORDER}`, borderRadius: '1.25rem', background: ROSE_DIM }}>
            <RefreshCw size={32} className="animate-spin mb-4" style={{ color: ROSE }} />
            <p className="font-medium text-slate-700">Analyzing risk signals...</p>
          </div>
        )}
        ...
        {approved && (
          ...
          >
            ...
            <p className="text-lg font-black text-slate-800 mb-1">Outreach Approved & Sent</p>
            <p className="text-slate-600 text-sm mb-4">
              Email dispatched to {selectedStudent.name} · {selectedStudent.advisorName} notified
            </p>
            <div
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold"
              style={{ background: 'rgba(255,255,255,0.7)', color: GREEN, border: `1px solid ${GREEN_BORDER}` }}
            >
              <CheckCircle size={12} />
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-xl" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
              ...
              <div className="space-y-2">
                {result.interventionPlan.map((a, i) => (
                  <div key={i} className="flex items-center gap-3 py-2 border-b last:border-b-0" style={{ borderColor: DIVIDER }}>
                    <span className="text-xs font-bold px-2 py-0.5 rounded"
                      style={{ background: `${urgencyColor[a.urgency] ?? '#64748b'}14`, color: urgencyColor[a.urgency] ?? '#64748b' }}>
                      {a.urgency}
                    </span>
                    <span className="flex-1 text-sm text-slate-700">{a.action}</span>
                    <span className="text-xs capitalize px-2 py-0.5 rounded"
                      style={{ background: `${ownerColor[a.owner] ?? '#64748b'}14`, color: ownerColor[a.owner] ?? '#64748b' }}>
                      {a.owner}
                    </span>
                  </div>
                ))}
            ...
            <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${BORDER}` }}>
              <button onClick={() => setEmailOpen(o => !o)}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-slate-700">AI-Drafted Intervention Email</span>
                  <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: AMBER_DIM, color: AMBER, border: `1px solid ${AMBER_BORDER}` }}>
                    Review & edit
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  {emailOpen ? <ChevronUp size={16} className="text-slate-400" /> : <ChevronDown size={16} className="text-slate-400" />}
                </div>
              </button>
              {emailOpen && (
                <div className="px-4 pb-4 pt-2 space-y-3" style={{ background: CARD_INNER }}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        style={{ background: editingEmail ? AMBER_DIM : CARD, color: editingEmail ? AMBER : '#64748b', border: `1px solid ${editingEmail ? AMBER_BORDER : BORDER}` }}
                      >
                  ...
                  {editingEmail ? (
                    <textarea
                      ...
                      rows={12}
                      className="w-full rounded-xl border text-sm text-slate-700 leading-relaxed resize-none p-3 focus:outline-none"
                      style={{ background: '#fff', borderColor: AMBER_BORDER }}
                    />
                  ) : (
                    <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{emailDraft}</p>
                  )}
                </div>
              )}
            </div>
            ...
            >
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-800 mb-0.5">Ready to send?</p>
  ...
  return (
    <div className="min-h-screen" style={{ background: '#ffffff', color: '#0f172a' }}>
      ...
      <div className="border-b" style={{ borderColor: '#e2e8f0' }}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/retain-ai" className="text-sm text-slate-400 hover:text-slate-600 transition-colors">RetainAI</Link>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="mb-10">
          <h1 className="text-3xl font-black tracking-tight mb-2 text-slate-900">RetainAI — Student Retention Intelligence</h1>
        ...
        <div className="mt-16 pt-8 border-t flex justify-between items-center" style={{ borderColor: '#e2e8f0' }}>
          <Link to="/beacon/demo" className="text-sm text-slate-400 hover:text-slate-600 transition-colors flex items-center gap-1">← Beacon Demo</Link>
