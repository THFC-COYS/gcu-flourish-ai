...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      'pAIgeBreaker watches what the student is reading in real time. The moment they hit something confusing — a dense paragraph, an unfamiliar term, a concept that doesn\'t click — it answers in context. No search. No tab switching. The explanation appears exactly where they need it.',
    bullets: [
      'Reads alongside every student, on every page',
      'Answers in context — no tab switching, no search',
      'Works on Canvas content, PDFs, and web readings',
      'Tracks comprehension patterns across the course',
    ],
    visual: 'paigebreaker',
  },
];
...
function ToolVisual({ type }: { type: string }) {
  ...
  if (type === 'paigebreaker') return (
    <div className="rounded-2xl border border-molted-border bg-molted-elevated p-5 space-y-3">
      <div className="flex items-center justify-between mb-1">
        <p className="text-molted-white text-sm font-semibold">Week 3 Reading · Cognitive Load Theory</p>
        <div className="flex items-center gap-1.5 text-xs" style={{ color: '#E8A020' }}>
          <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: '#E8A020' }} />
          Reading
        </div>
      </div>
      <div className="bg-molted-surface rounded-xl p-4 border border-molted-border text-xs text-molted-muted leading-relaxed">
        <span className="text-molted-white">…Intrinsic cognitive load refers to the inherent complexity of the material being learned, which cannot be altered by instructional design. Extraneous cognitive load, by contrast,</span>
        {' '}<span className="rounded px-1 py-0.5" style={{ background: 'rgba(232,160,32,0.2)', color: '#E8A020' }}>is generated by poorly designed instruction</span>{' '}
        <span className="text-molted-white">and can be reduced through thoughtful…</span>
      </div>
      <div
        className="rounded-xl p-4 border text-xs leading-relaxed"
        style={{ background: 'rgba(232,160,32,0.06)', borderColor: 'rgba(232,160,32,0.2)', color: '#F5F5F7' }}
      >
        <p className="text-xs font-semibold mb-1.5" style={{ color: '#E8A020' }}>pAIgeBreaker</p>
        Extraneous load is the mental effort wasted on <em>how</em> something is taught rather than <em>what</em> is being taught. Think of it like background noise — the harder your textbook is to follow, the more brain energy you spend decoding the writing instead of learning the concept. Good instructional design eliminates that noise.
      </div>
    </div>
  );
  ...
  return (
    <section className="py-28 px-6 border-t border-molted-border">
      <div className="max-w-6xl mx-auto">
        <RevealBlock className="text-center mb-16">
          <p className="text-molted-muted text-sm font-semibold uppercase tracking-widest mb-4">Six core tools</p>
          <h2 className="text-4xl md:text-6xl font-black text-molted-white tracking-tight leading-tight">
        ...
        <div className="grid md:grid-cols-[340px_1fr] gap-8 items-start">
          ...
          <div className="space-y-6">
            ...
            {active.visual === 'paigebreaker' && (
              ...
              >
                <Play size={14} />
                Explore pAIgeBreaker — the reading companion
                <ArrowRight size={14} />
              </Link>
            )}
          </div>
