...
const MOCK_TESTIMONIALS = [
  {
    ...
    avatar: 'J',
    color: '#E0F2FE',
    story: 'I once sat with a frightened 84-year-old woman for 45 minutes before her surgery — not because protocol required it, but because she needed someone to hold her hand and pray with her. That\'s what YJU trained me to do. I want every AI that speaks to a patient to carry that same impulse.',
...
const HOW_IT_WORKS = [
  { step: '01', title: 'Alumni Contribute', desc: 'YJU graduates submit defining stories, ethical decisions, and wisdom through the Spirit Network portal.', icon: '🎓' },
    ...
    <div className="page-card p-5 flex flex-col gap-4 hover:shadow-card-hover transition-all">
      ...
      <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed italic">
        ...
        {t.story.length > 140 && (
          <button
            ...
            className="ml-1 text-yju-primary dark:text-purple-400 font-semibold not-italic hover:underline"
          >
            {expanded ? 'less' : 'more'}
          </button>
        )}
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-[#2D2050]">
        <div className="flex items-center gap-1.5">
          <Zap size={11} className="text-yju-primary" />
          <span className="text-xs text-yju-primary dark:text-purple-400 font-semibold">{t.vessel.replace('YJU ', '').replace('Spirit ', '')}</span>
        </div>
...
export default function SpiritNetwork() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* ── HERO ── */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-yju-primary via-yju-primary-dark to-[#1A0F35] p-8 sm:p-10">
        <svg className="absolute bottom-0 right-0 opacity-10 w-64 h-64" viewBox="0 0 200 200">
          <circle cx="100" cy="100" r="80" fill="none" stroke="#E85D04" strokeWidth="2" />
          <circle cx="100" cy="100" r="55" fill="none" stroke="#E85D04" strokeWidth="1.5" />
          <circle cx="100" cy="100" r="30" fill="none" stroke="#E85D04" strokeWidth="1" />
        </svg>
        <div className="relative z-10 max-w-2xl">
          <div className="inline-flex items-center gap-2 bg-yju-accent/20 border border-yju-accent/30 text-yju-accent text-xs font-bold px-3 py-1.5 rounded-full mb-4 uppercase tracking-widest">
            <Heart size={11} />
            The Spirit Network
          </div>
          <h1 className="text-3xl sm:text-4xl font-black text-white mb-3">
            Your Spirit,<br />
            <span className="text-yju-accent">Multiplied Infinitely.</span>
          </h1>
          <p className="text-white/70 text-base leading-relaxed mb-6">
            YJU graduates are not just alumni — they are the living source code of our AI.
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={s.label} className="text-center">
                <div className="text-2xl font-black text-yju-accent">{s.value}</div>
            ...
            <div key={step.step} className="relative">
              <div className="flex items-start gap-3">
                <div className="flex flex-col items-center">
                  <div className="w-10 h-10 rounded-xl bg-yju-primary flex items-center justify-center text-yju-accent font-black text-sm flex-shrink-0">
                    {step.step}
                  </div>
                </div>
                <div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs bg-yju-primary/10 text-yju-primary dark:text-purple-300 px-2 py-0.5 rounded-full font-semibold">{MOCK_TESTIMONIALS.length} featured</span>
          </div>
      ...
      <div className="page-card p-6">
        <div className="flex items-start gap-4 mb-6">
          <div className="w-11 h-11 rounded-xl bg-yju-accent/20 flex items-center justify-center flex-shrink-0">
            <Heart size={20} className="text-yju-accent" />
          </div>
          <div>
            <h2 className="text-base font-black text-slate-900 dark:text-white">Contribute Your Spirit</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              Every YJU alumnus can submit wisdom, stories, and ethical decisions to living spirit vessels.
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="form-label">YJU College *</label>
              <select
                className="form-input"
            ...
            <div className="sm:col-span-2">
              ...
              <div className="flex gap-2 mb-2">
                <button type="button" className="flex items-center gap-1.5 text-xs text-yju-primary dark:text-purple-400 border border-yju-primary/30 px-3 py-1.5 rounded-lg hover:bg-yju-primary-pale transition-colors">
            ...
            <div className="sm:col-span-2">
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  className="mt-0.5 w-4 h-4 rounded accent-yju-primary"
                  checked={formData.consent}
                  onChange={e => setFormData(f => ({ ...f, consent: e.target.checked }))}
                  required
                />
                <span className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                  I consent to YJU using my story and wisdom to train and enrich spirit vessel AI models. I understand my contribution will be attributed to me by name (with my approval), and I will receive impact reports showing how many people my wisdom has served. YJU will never sell or share my contribution outside the Empyrean LMS platform without my explicit consent.
                </span>
              </label>
            </div>
            <div className="sm:col-span-2">
      ...
      <div className="page-card p-6">
        <div className="flex items-center gap-2 mb-4">
          <Star size={16} className="text-yju-accent" />
        ...
        <div className="space-y-2">
          {MOCK_TESTIMONIALS.sort((a, b) => b.interactions - a.interactions).map((t, i) => (
            <div key={t.id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-[#1A1235] border border-transparent hover:border-yju-primary/20 transition-colors">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 ${
                i === 0 ? 'bg-yju-accent text-yju-primary-dark' :
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{t.vessel.replace('YJU ', '')}</p>
              </div>
              <div className="flex items-center gap-1">
                <Users size={12} className="text-yju-primary" />
                <span className="text-sm font-black text-yju-primary dark:text-purple-300">{t.interactions.toLocaleString()}</span>
              </div>
            </div>
          ))}
        </div>
