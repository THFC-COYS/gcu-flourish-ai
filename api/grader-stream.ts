/*
  TeachOS · Agentic Grader — SSE streaming endpoint
  POST { rubric, submission, assignmentTitle?, assignmentType?, totalPoints?, voice? }
  Streams: event: trace | criterion | result | error
*/
...
export default async function handler(req: any, res: any) {
  ...
  if (!rubric || String(rubric).trim().length < 20) {
    return res.status(400).json({ error: 'Please provide a rubric.' });
  }
  if (!submission || String(submission).trim().length < 30) {
    return res.status(400).json({ error: 'Please provide a student submission.' });
  }
...
Grade each rubric criterion individually. Cite specific passages from the student's work in your feedback. Assign scores that reflect the rubric criteria exactly.`;
  ...
  try {
    // Run Grok API call and trace animation in parallel
    ...
    // Emit criteria one by one with a short delay for dramatic reveal
